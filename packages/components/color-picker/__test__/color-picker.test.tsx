...
//   return {
//     ...((await vi.importActual('lodash')) as Record<string, any>),
//     debounce: vi.fn((fn) => {
//       fn.cancel = vi.fn()
//       fn.flush = vi.fn()
//       return fn
...
describe('Color-picker', () => {
  it('Mount Color-picker', () => {
    ...
    expect(wrapper.html()).toContain('n-color-picker')
    wrapper.unmount()
  })
  it('should show alpha slider when show-alpha=true', async () => {
    ...
    await wrapper.find('.n-color-picker__trigger').trigger('click')
    const alphaSlider = document.querySelector('.n-color-alpha-slider')
  ...
  it('control alpha changes through keyboard', async () => {
    ...
    await wrapper.find('.n-color-picker__trigger').trigger('click')
    const alphaSlider = wrapper.findComponent('.n-color-alpha-slider')
    await alphaSlider.find('.n-color-alpha-slider__thumb').trigger('keydown', {
      key: 'ArrowDown',
      code: 'ArrowDown'
    })
    await alphaSlider.find('.n-color-alpha-slider__thumb').trigger('keydown', {
  ...
  it('should pick a color when confirm button click', async () => {
    ...
    await wrapper.find('.n-color-picker__trigger').trigger('click')
    document.querySelector<HTMLElement>('.n-color-dropdown__btn')?.click()
    await nextTick()
  ...
  it('should show color picker when click trigger', async () => {
    ...
    await wrapper.find('.n-color-picker__trigger').trigger('click')
    const dropdown = document.querySelector('.n-color-dropdown')
  ...
  it('should change color to the selected color', async () => {
    ...
    expect(
      predefineDom.querySelectorAll('.n-color-predefine__color-selector')
        .length === 9
    ).toBeTruthy()
    predefineDom
      .querySelector<HTMLElement>(
        '.n-color-predefine__color-selector:nth-child(4)'
      )
      ?.click()
    await nextTick()
    ...
    predefineDom
      .querySelector<HTMLElement>(
        '.n-color-predefine__color-selector:nth-child(3)'
      )
      ?.click()
    await nextTick()
  ...
  it('should change selected state of predefined color', async () => {
    ...
    expect(
      predefineWrapper
        .find('.n-color-predefine__color-selector:nth-child(4)')
        .classes()
    ).toContain('selected')
    const hueSlideWrapper = colorPickerWrapper.findComponent({ ref: 'hue' })
    const hueSlideDom = hueSlideWrapper.element
  ...
  it('should update the colorFormat and selected color when the colorFormat prop changes', async () => {
    ...
    expect(
      customInput.find<HTMLInputElement>('.n-input__inner').element.value
    ).toBe('rgb(0, 255, 0)')
    wrapper.unmount()
  })
  ...
  it('it will target the focus & blur', async () => {
    ...
    await nextTick()
    await wrapper.find('.n-color-picker').trigger('focus')
    expect(focusHandler).toHaveBeenCalledTimes(1)

    await wrapper.find('.n-color-picker').trigger('blur')
